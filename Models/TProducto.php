...
trait TProducto{
	...
	public function getProductosT(){
		...
		$sql = "SELECT p.idproducto,
				...
				WHERE p.status != 0 ORDER BY p.idproducto DESC LIMIT ".CANTPRODUCTOS;
	...
	public function getProductoT(int $idproducto, string $ruta){
		...
		$sql = "SELECT p.idproducto,
				...
				ON p.categoriaid = c.idcategoria
				WHERE p.status != 0
				AND p.idproducto = '{$this->intIdProducto}' AND p.ruta = '{$this->strRuta}'";
	...
	public function getProductosRandom(int $idcategoria, int $cant, string $opcion){
		$this->intIdcategoria = $idcategoria;
		$this->cant = $cant;
		$this->opcion = $opcion;
		...
		if($opcion == "r"){
			...
			$this->opcion = " idproducto ASC ";
		} else {
			$this->opcion = " idproducto DESC ";
		}
		...
		$sql = "SELECT p.idproducto,
				...
				ORDER BY $this->opcion LIMIT $this->cant";
				$request = $this->con->select_all($sql);
				if(count($request) > 0){
					for ($c=0; $c < count($request) ; $c++) { 
						$intIdProducto = $request[$c]['idproducto'];
