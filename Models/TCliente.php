...
trait TCliente{
	...
	public function insertPedido(string $idtransaccionpaypal = NULL, string $datospaypal = NULL, int $personaid, float $costoenvio, float $monto, int $tipopagoid, string $direccionenvio, string $status){
		$this->con = new Mysql();
	...
	public function insertDetalleTemp(array $pedido){
		...
		$sql = "SELECT * FROM detalle_temp 
					WHERE transaccionid = '{$this->intIdTransaccion}' 
					AND personaid = $this->intIdUsuario";
		$request = $this->con->select_all($sql);
		if(empty($request)){
