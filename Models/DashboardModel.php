		...
		public function selectVentasMes(int $anio, int $mes){
			...
			for ($i=0; $i < $dias; $i++) {
				...
				$sql = "SELECT DAY(fecha) AS dia, COUNT(idpedido) AS cantidad, SUM(monto) AS total
						FROM pedido
						WHERE DATE(fecha) = '$fechaVenta' AND status = 'Completo' ".$where." GROUP BY DAY(fecha)";
				$ventaDia = $this->select($sql);
				$ventaDia['dia'] = $n_dia;
				$ventaDia['dia'] = $ventaDia['dia'] == "" ? 0 : $ventaDia['dia'];
				$ventaDia['cantidad'] = $ventaDia['cantidad'] == "" ? 0 : $ventaDia['cantidad'];
